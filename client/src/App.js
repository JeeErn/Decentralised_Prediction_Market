...
function App() {
  ...
  const { userType, reputation, arbitratorName } = useGetUserInfo({ predictionMarketInstance, accountAddress: accounts?.[0] });
  ...
  return (
    <>
      ...
      {(userType !== 'Invalid' && <Navbar userType={userType} reputation={reputation} arbitratorName={arbitratorName} />) }
      {(userType === 'Trader and Arbitrator' || userType === 'Trader') && predictionMarketInstance && <CreateTopic predictionMarketInstance={predictionMarketInstance} accountAddress={accounts?.[0]} />}
